      ...
      const render = () => {
        ...
        if (tab === 'list') {
          ...
          html += `<div style="max-height:60vh;overflow:auto"><table class="list"><tr><th>武将</th><th>所在</th><th>年</th>${th('war', '武')}${th('lead', '統')}${th('pol', '政')}${th('cha', '魅')}${th('rank', '位階')}<th>特技</th><th>相性</th>${th('loyalty', '忠誠→目標')}<th></th></tr>
            ${gens.map((g) => {
              ...
              return `<tr><td>${isRuler ? '👑' : ''}${esc(g.name)}${role ? ` <span class="role-tag">${ROLES[role].name}</span>` : ''}${g.family && !isRuler ? ' <span class="muted small">一門</span>' : ''}</td>
                ...
                <td class="row">${g.exposed && g.betray ? `<button class="btn small" data-ask="${g.id}" title="成功すれば二心を捨てる。失敗すると兵を連れて出奔する">詰問</button>${chanceText(st, nid, interrogateChance(st, g), `ask:${g.id}`)}<button class="btn small" data-ban="${g.id}">追放</button>` : ''}${canPromote(g) ? `<button class="btn small primary" data-promote="${g.id}">昇進（${promoteCost(g)}金）</button>` : ''}
          ...
          html += `<h3>特技</h3><table class="list">${Object.values(TRAITS).map((t) => `<tr><td style="white-space:nowrap"><span class="trait">${t.name}</span></td><td>${esc(t.desc)}</td></tr>`).join('')}</table>
            ...
            <h3>軍師</h3><p class="muted">成功率は数字では示されません。軍師がいると、外交・登用・調略・計略などの見込みを「うまくいくでしょう」「難しいでしょう」といった言葉で助言してくれます（政治力が低いと見立てが外れることもあります）。軍師は敵の調略を見破り、離間の計（外交画面）や流言（敵地の画面）を仕掛け、毎季、地図の左上で助言をします。</p>
        ...
        el.innerHTML = `<p class="muted">隣接する地方にいる自軍の武将を使者として送り、敵将を誘います（${SUBVERT_COST}金・使者はその季節は動けません）。<br>
          ...
          ${targets.map((g) => {
            ...
            const btn = (mode) => {
              ...
              return `<button class="btn small" data-sv="${g.id}" data-mode="${mode}" ${tried || nat.gold < SUBVERT_COST ? 'disabled' : ''}>仕掛ける</button>${chanceText(st, nid, p, `sv:${agent.id}:${g.id}:${mode}`)}`;
            };
